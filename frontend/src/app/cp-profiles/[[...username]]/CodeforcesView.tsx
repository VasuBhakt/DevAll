...
export default function CodeforcesView({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cp-profiles", username] });
      alert("CodeforcesProfile deleted successfully!");
    },
    onError: (error: any) => {
  ...
  const rankColor = (rank: number) => {
    ...
    if (rank < 1400)
      return { first: "text-green-500", second: "text-green-500" };
              ...
              {profile && (
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-10">
                  <StatItem
                    ...
                    color={rankColor(profile.rating)}
                  />
                  <StatItem
                    icon={Award}
                    label="Max Rating"
                    ...
                    color={rankColor(profile.max_rating)}
                  />
                  <StatItem
                    icon={History}
                    label="Contests"
