...
import { CodechefProfile } from "@/services/cpProfile/profiles/codechef";
...
export const CodechefRankColor = (
  rank: number
): { first: string; second: string; rank: string } => {
  if (rank < 1400)
  ...
  if (rank < 2500)
    return { first: "text-orange-500", second: "text-orange-500", rank: "6" };
  return { first: "text-red-500", second: "text-red-500", rank: "7" };
};

export default function CodechefView({
  profile,
  isOwner,
  username,
}: CodechefViewProps) {
  ...
  const syncMutation = useMutation({
    mutationFn: (handle: string) =>
      CPService.fetchCPProfile("codechef", handle),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cp-profiles", username] });
      alert("Codechef Profile synchronized successfully!");
  ...
  const deleteMutation = useMutation({
    mutationFn: () => {
      if (confirm("Are you sure you want to delete your Codechef profile?")) {
        return CPService.deleteCPProfile("codechef");
      }
      return Promise.reject(new Error("Profile not deleted"));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cp-profiles", username] });
      alert("Codechef Profile deleted successfully!");
    },
    onError: (error: any) => {
    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700 ease-out">
      ...
      <div className="relative p-8 md:p-12 rounded-[2.5rem] bg-card/40 backdrop-blur-xl border border-border/60 shadow-2xl overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-40 transition-opacity duration-700 pointer-events-none">
          <img src="/codechef.png" alt="CodeChef" className="w-76 h-32" />
        </div>
              ...
              {profile && (
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-10">
                  <StatItem
                    ...
                    color={CodechefRankColor(profile.rating)}
                  ...
                  <StatItem
                    ...
                    color={CodechefRankColor(profile.max_rating)}
                    subValue={CodechefRankColor(profile.max_rating).rank}
                  />
                  <StatItem
                    icon={History}
                    label="Contests"
...
function StatItem({
  ...
  return (
    <div className="flex flex-col items-center md:items-start gap-1 group/stat transition-all">
      ...
      <div className={cn("flex items-center gap-1", color.first)}>
        {subValue && (
          <>
            <span className="pl-3 text-2xl font-semibold tabular-nums">
              <StarIcon size={16} />
            </span>
            <span
              className={cn("text-2xl font-semibold tabular-nums", color.first)}
