                  ...
                  {profile.organizations.map((org, i) => (
                    <>
                      ...
                      {profile.organizations?.length &&
                        i < profile.organizations.length - 1 && (
...
function RepoCard({ repo }: { repo: GithubPinnedRepo }) {
  return (
    <div className="group/repo p-8 rounded-[2.5rem] bg-card/30 backdrop-blur-sm border border-border/40 hover:border-primary/40 hover:shadow-2xl transition-all flex flex-col justify-between relative overflow-hidden">
      <div className="absolute top-0 left-0 w-1.5 h-full bg-primary/10 group-hover/repo:bg-primary/50 transition-colors" />
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between mt-8 pt-6 border-t border-border/10 relative z-10">
        <a
          href={repo.url}
          target="_blank"
          rel="noopener noreferrer"
